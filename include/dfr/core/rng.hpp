 ...
 public:
  ...
  [[nodiscard]] constexpr std::uint64_t below(std::uint64_t bound) noexcept {
    ...
    if (low < bound) {
      ...
      // threshold is fixed for the whole loop, but low is not: it is recomputed from a fresh draw every
      // iteration, which is the actual controlling variable, and clang-tidy's dataflow does not follow it
      // through wide_multiply. Bounded in expectation, not absolutely, per the comment above this function.
